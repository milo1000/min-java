...
public class MinTransport {
    ...
    private final int ack_retransmit_timeout_ms = 25;
    private final int max_window_size = 8;
    private final int idle_timeout_ms = 3000;
    private final int frame_retransmit_timeout_ms = 50;
     ...
     */
    public void sendFrame(final int minId, final byte[] payload) throws MinIdException, PayloadTooLongException {
    ...
    public ArrayList<MinFrame> poll() {
        ...
        if ((window_size < max_window_size) && transport_fifo.size() > window_size) {
            ...
            if ((window_size > 0) && remote_connected) {
                ...
                if (currentTimeMillis - oldest_frame.last_sent_time > frame_retransmit_timeout_ms) {
        ...
        if (currentTimeMillis - last_sent_ack_time_ms > ack_retransmit_timeout_ms) {
            if (remote_active) {
                System.out.println("sending ACK periodic");
                send_ack();
            }
        }
